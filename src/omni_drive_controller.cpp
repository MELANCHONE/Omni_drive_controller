...
using ControllerReferenceMsg =
  omni_drive_controller::OmniDriveController::ControllerReferenceMsg;
using ControllerReferenceMsgUnstamped =
  omni_drive_controller::OmniDriveController::ControllerReferenceMsgUnstamped;
...
controller_interface::CallbackReturn OmniDriveController::on_init()
{
  try
  {
    param_listener_ = std::make_shared<omni_drive_controller::ParamListener>(get_node());
  }
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  odometry_.setWheelsParams(
    params_.kinematics.wheel_to_center_distance,
  ...
  if (use_stamped_vel_)
  {
    ref_subscriber_ = get_node()->create_subscription<ControllerReferenceMsg>(
      ...
      std::bind(&OmniDriveController::reference_callback, this, std::placeholders::_1));
  }
  else
  {
    ref_unstamped_subscriber_ = get_node()->create_subscription<ControllerReferenceMsgUnstamped>(
      ...
      std::bind(
        &OmniDriveController::reference_unstamped_callback, this, std::placeholders::_1));
  }
  ...
  try
  {
    transform_stamped_pub_ = get_node()->create_publisher<geometry_msgs::msg::TransformStamped>(
      "~/transform", rclcpp::SystemDefaultsQoS());
    rt_transform_stamped_publisher_ = std::make_unique<TransformStampedPublisher>(transform_stamped_pub_);
  }
  ...
  rt_transform_stamped_publisher_->lock();
  rt_transform_stamped_publisher_->msg_.header.frame_id = params_.odom_frame_id;
  rt_transform_stamped_publisher_->msg_.child_frame_id = params_.base_frame_id;
  rt_transform_stamped_publisher_->msg_.transform.translation.z = 0.0;
  rt_transform_stamped_publisher_->unlock();
  RCLCPP_INFO(get_node()->get_logger(), "OmniDriveController configured successfully");

  return controller_interface::CallbackReturn::SUCCESS;
}

void OmniDriveController::reference_callback(const std::shared_ptr<ControllerReferenceMsg> msg)
{
...
void OmniDriveController::reference_unstamped_callback(
...
OmniDriveController::command_interface_configuration() const
{
...
controller_interface::InterfaceConfiguration OmniDriveController::state_interface_configuration()
  const
{
...
OmniDriveController::on_export_reference_interfaces()
{
...
bool OmniDriveController::on_set_chained_mode(bool /*chained_mode*/) { return true; }
...
controller_interface::return_type OmniDriveController::update_reference_from_subscribers()
{
...
controller_interface::return_type OmniDriveController::update_and_write_commands(
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
    ...
    !std::isnan(reference_interfaces_[2]))
  {
    ...
    // 中心速度转换到各个轮子速度
    // 左前轮速度 = 1/r * (v_x + v_y - k * v_theta)
    const double sqrt2_2 = 0.7071067811865476;  // √2/2

    const double wheel_front_left_vel = (1.0 / params_.kinematics.wheels_radius) *
      (sqrt2_2 * (-velocity_in_center_frame_linear_x_ + velocity_in_center_frame_linear_y_) +
       params_.kinematics.wheel_to_center_distance * velocity_in_center_frame_angular_z_);
    
    const double wheel_front_right_vel = (1.0 / params_.kinematics.wheels_radius) *
      (sqrt2_2 * (velocity_in_center_frame_linear_x_ + velocity_in_center_frame_linear_y_) +
       params_.kinematics.wheel_to_center_distance * velocity_in_center_frame_angular_z_);
                                                                           
    const double wheel_rear_left_vel = (1.0 / params_.kinematics.wheels_radius) *
      (sqrt2_2 * (-velocity_in_center_frame_linear_x_ - velocity_in_center_frame_linear_y_) +
       params_.kinematics.wheel_to_center_distance * velocity_in_center_frame_angular_z_);
    
    const double wheel_rear_right_vel = (1.0 / params_.kinematics.wheels_radius) *
      (sqrt2_2 * (velocity_in_center_frame_linear_x_ - velocity_in_center_frame_linear_y_) +
       params_.kinematics.wheel_to_center_distance * velocity_in_center_frame_angular_z_);
  ...
  if (rt_transform_stamped_publisher_->trylock())
  {
    rt_transform_stamped_publisher_->msg_.header.stamp = time;
    rt_transform_stamped_publisher_->msg_.header.frame_id = params_.odom_frame_id;
    rt_transform_stamped_publisher_->msg_.child_frame_id = params_.base_frame_id;
    rt_transform_stamped_publisher_->msg_.transform.translation.x = odometry_.getX();
    rt_transform_stamped_publisher_->msg_.transform.translation.y = odometry_.getY();
    rt_transform_stamped_publisher_->msg_.transform.translation.z = 0.0;
    rt_transform_stamped_publisher_->msg_.transform.rotation = tf2::toMsg(orientation);
    rt_transform_stamped_publisher_->unlockAndPublish();
  }
